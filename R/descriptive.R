...
# Statistiques descriptives de base g?n?r?es automatiquement    
  
  Y <- histfreq.0(X=X,dfreq=dfreq,dtype=dtype,vt=t) # Nombre d'individus captur?s i fois
  nbrecapt <- rev(Y)
  if (dtype=="hist") {        
    premcapt <- getui(X=X,dfreq=dfreq,t=t) # Nombre d'individus captur?s pour la premi?re fois ? l'occasion i        
    derncapt <- getvi(X=X,dfreq=dfreq,t=t) # Nombre d'individus captur?s pour la derni?re fois ? l'occasion i
    captoccas <- getni(X=X,dfreq=dfreq,t=t) # Nombre d'individus captur?s ? l'occasion i
  }       
...
# Matrice de recapture g?n?r?e mais pas dans le print
...
plot.descriptive <- function(x,main="Exploratory Heterogeneity Graph", ...){
  ...
  # Pr?paration du premier graphique : celui des fi
  ...
  # Pr?paration du deuxi?me graphique : celui des ui 
  ...
  on.exit(par(op)) ## Pour remettre les param?tres graphiques par d?faut ? la sortie de la fonction
  ...
  if (ngraph==2) mtext(main,side=3,line=2.7,cex=1.8) # S'il y a deux graphiques, le texte doit ?tre ajout? apr?s le premier graphique
  if (dim(x$base.freq)[2]==4) {
...
{ 
  # Nombre d'individus captur?s i fois
  nbrecapt <- rep(0,t) # On veut avoir les fr?quences pour tous les nbcap, m?me les fr?quences nulles
...
{ 
  # Nombre d'individus captur?s pour la premi?re fois ? l'occasion i
  premcapt <- rep(0,t)
  for(i in (1:dim(X[,1:t])[1])) {
    k <- 0
    j <- 1
...
{ 
  # Nombre d'individus captur?s pour la derni?re fois ? l'occasion i
  derncapt <- rep(0,t)
  for(i in (1:dim(X[,1:t])[1])) {
    k <- 0
    j <- t
...
{ 
  # Nombre d'individus captur?s ? l'occasion i
  if (dfreq) {
    captoccas <- rep(0,t)
