...
closedp.Mtb <- function(X, dfreq = FALSE, method = "BFGS", ...)
{
  ...
  # matrices d'indicatices de non capture (wi=0) avant la premi?re capture, i=1,...,t-1
  I1 <- matrix(c(0,0,1),ncol=1)  
  for(i in (3:t))
    I1 <- cbind(c(rep(0, 2^(i-1)), rep(1,((2^(i-1))-1))), 
                rbind(matrix(rep(0,(2^(i-1))*(i-2)),nrow=2^(i-1)),I1))
  # matrice d'indicatrices de la premi?re capture
  ...
  # matrice d'indicatrices de capture (wi=1) apr?s la premi?re capture
  I3 <- histpos.t(t)[,-1]  
  for(i in (2^(t-1)+1):(2^t-1)) 
    I3[i,1:(poscapt1[i]-1)]<-rep(0,length(I3[i,1:(poscapt1[i]-1)]))
  # matrice d'indicatrices de non-capture (wi=0) apr?s la premi?re capture
  I4 <- 1-histpos.t(t)[,-1]  
  for(i in (2^(t-1)+1):(2^t-1)) 
    I4[i,1:(poscapt1[i]-1)]<-rep(0,length(I4[i,1:(poscapt1[i]-1)]))
  
  # valeurs initiales des param?tres (N->n, tous les p -> 0.5 et ci=pi)
  ...
  # Optimisation de la fonction de log-vraisemblance en fonction des param?tres loglin?aires
  optim.out <- optim.call(par = init_loglinparam.Mtb, fn = ll_loglinparam.Mtb, method=method,
                          Y = Y, I1 = I1, I2 = I2, I3 = I3, I4 = I4, 
                          control=list(fnscale = -1, maxit = 1000), ...)
  
  # Si la commande ? g?n?r? une erreur, on arr?te l'ex?cution de la fonction
  if (!is.null(optim.out$error))
    stop("error when fitting the model: ", optim.out$error)  
  
  # Statistiques d'ajustement du mod?le
